...
public class PraiseCommand implements Command {
	...
	public static final String COMMAND_TEXT = "praise";
	...
	public void executeCommand(LinkedList<String> args, MessageReceivedEvent event) {
		if (isOdd) {
			...
			event.getTextChannel().sendMessage("It is appreciated, " + name + ".  Sorazae++");
		} else {
			String name = event.getAuthorName();
			event.getTextChannel().sendMessage("Thank you, " + name + ".  Sorazae++");
		}
		isOdd = !isOdd;
	}
